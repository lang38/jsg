...
class ModuleObject extends MasterObject
{
	...
	{
				include_once ROOT_PATH.'include/func/zip.moyo.php';
		$this->upsDataDIR = ROOT_PATH.'data/upgrade/';
		jio()->initPath($this->upsDataDIR);
		
		jio()->MakeDir($this->upsDataDIR . 'upgrade/');
		jio()->MakeDir(ROOT_PATH . 'install/udata/upgrade/');
	}
	...
	function Main()
	{
				$dir_list=array("api","data","data/upgrade","data/backup","data/cache","data/log","images","include","install","modules","plugin","setting","templates","theme","wap","./",);
	...
	function check()
	{
		...
		else
		{
			...
			include template('admin/upgrade_change_log');
			exit;
		}
	}
	...
	function download()
	{
		...
		is_dir($upgrade_data_dir) || jio()->MakeDir($upgrade_data_dir);
		...
		$ret = jio()->WriteFile($tmp_file, $data, $mode);
		if(!$ret) {
	...
	function install()
	{
		...
		is_dir($upgrade_tmp_dir) || jio()->MakeDir($upgrade_tmp_dir);
				...
				if($step=='check')
		{
			...
			include template('admin/upgrade_change_list');
			exit;
		}
				...
				if ($step=='backup')
		{
			$original_path=ROOT_PATH;			$backup_path=ROOT_PATH.'data/backup/'.SYS_VERSION.'-'.SYS_BUILD.'/';			if(!is_dir($backup_path)) {
				jio()->MakeDir($backup_path,0777);
			}
			clearstatcache();

			$files = jio()->ReadDir($upgrade_tmp_dir, 1);
			...
			foreach ($files as $i => $upfile_path)
			{
				...
				$webfile_path = jio()->initPath(ROOT_PATH.$baseFileName);
				$bakfile_path = jio()->initPath($backup_path.$baseFileName);
								if (is_file($webfile_path))
				{
					jio()->CopyFile($webfile_path, dirname($bakfile_path)) || $unbackup_file_list[$webfile_path] = $bakfile_path;
				}
																				$const_file = 'setting/constants.php';
				if (substr($baseFileName, -strlen($const_file)) != $const_file)
				{
					
					jio()->CopyFile($upfile_path, dirname($webfile_path)) || $unbackup_file_list[$upfile_path] = $webfile_path;
				}
			}
		...
		$upgrade_script_dir = $upgrade_tmp_dir.'__upgrade__/';
			...
			$files = jio()->ReadDir($upgrade_script_dir);
		...
		is_file($copy_from) && jio()->CopyFile($copy_from, dirname($copy_to));
